...
func resourceVPCPeeringConnectionCreate(ctx context.Context, d *schema.ResourceData, m interface{}) diag.Diagnostics {
	...
	if pc.State != "ACTIVE" {
		switch pc.State {
		case "PENDING_PEER":
			return diag.Errorf("Aiven platform has created a connection to the specified "+
		...
		case "REJECTED_BY_PEER":
			return diag.Errorf("AWS VPC peering connection request was rejected, state info: %s",
				stateInfoToString(pc.StateInfo))
